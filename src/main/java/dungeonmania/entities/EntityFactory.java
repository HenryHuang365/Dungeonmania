...
import dungeonmania.entities.logicals.LogicalsFactory;
...
public class EntityFactory {
    ...
    public void spawnSpider(Game game) {
        ...
        map.getEntities(SwampTile.class)
            .stream()
            .filter(t -> t.getPosition().equals(spider.getPosition()))
            .forEach(t -> {
    ...
    public void spawnZombie(Game game, ZombieToastSpawner spawner) {
        ...
        map.addEntity(zt);
        //when zombie spawn on a swamp tile
        map.getEntities(SwampTile.class)
            .stream()
            .filter(t -> t.getPosition().equals(zt.getPosition()))
            .forEach(t -> {
                if (t.getMoveMentFactor() != 0) {
                    t.getStuck((zt));
                }
    ...
    public Sceptre buildSceptre() {
        int mindcontrolduration = config.optInt("mind_control_duration", 2);
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "wire":
            return new Wire(pos);
        case "switch_door":
            LogicalEntity logcalent =
            LogicalsFactory.buildLogicalEntity("switch_door", jsonEntity.getString("logic"), pos);
            return ((SwitchDoor) logcalent);
        case "light_bulb_off":
            LogicalEntity logicbulb =
            LogicalsFactory.buildLogicalEntity("light_bulb_off", jsonEntity.getString("logic"), pos);
            return ((LightBulb) logicbulb);
        default:
            return null;
        }
    }
